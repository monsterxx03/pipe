...
func TestBuildBPFFilter(t *testing.T) {
	// one host
	result := buildBPFFilter(false, false,
		[]string{"127.0.0.1"}, "80")
	...
	// udp
	result = buildBPFFilter(false, true, []string{"127.0.0.1"}, "80")
	assertEqual(t, result, "udp dst port 80 and ( dst host 127.0.0.1)")
	// track response
	result = buildBPFFilter(true, false, []string{"127.0.0.1"}, "80")
